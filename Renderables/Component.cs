...
public class Component : IRenderable
{
    ...
    public static bool TryCreate(Frustum frustum, ModelPipeline modelPipeline, GraphicsDevice graphicsDevice, CommandList commandList, ResourceSet cameraResourceSet, UObject component, UStaticMesh staticMesh, FTransform[] transforms, UObject[] overrideMaterials, out Component componentInstance)
    {
        ...
        try
        {
            var overrideMaterialsProcessed = overrideMaterials?.Where(m => m != null).Select(m =>
                ResourceCache.GetOrAdd(m.Outer?.Name ?? "Unknown", () => new Material(graphicsDevice, commandList, modelPipeline.MaterialResourceLayout, m))
            ).ToArray() ?? [];
            ...
            bool twoSided = component.Outer?.GetOrDefault<bool>("bMirrored") == true || component.GetOrDefault<bool>("bDisallowMeshPaintPerInstance") == true;
            ...
            componentInstance = new Component(frustum, modelPipeline, commandList, cameraResourceSet, createdModel, overrideMaterialsProcessed, twoSided, transformBuffer, (uint)transforms.Length)
            {
                Bounds = bounds
            };
        }
